...
public class ChromosomeShaker {
    ...
    public static List<Chromosome> middleCrossover(List<Chromosome> chromosomes, int n, int chromosomesInGen, double eliteRatio) {
        Random random = new Random();
        boolean[] alreadyCrossed = new boolean[chromosomesInGen];
        int idOfCrossingPair;
        Arrays.fill(alreadyCrossed, false);
        int startAt = (int)(chromosomesInGen*eliteRatio);
        //System.out.println("start at: " + startAt);
        for(int i=startAt;i<chromosomesInGen;i++) {
            if (!alreadyCrossed[i]) {
                do{
...
//                System.out.printf("i, idofcrossinpair: %d, %d\n", i, idOfCrossingPair);
    ...
    public static List<Chromosome> mutate(List<Chromosome> chromosomes, int n, int chromosomesPerGen, double eliteRatio) {

        int startAt = (int)(chromosomesPerGen*eliteRatio);
        for(int i=startAt; i< chromosomesPerGen;i++) {
            Gene firstGene = chromosomes.get(i).getGeneList().get((int)(Math.random()*n));
            Gene secondGene = chromosomes.get(i).getGeneList().get((int)(Math.random()*n));
    ...
    public static void setCmaxes(List<Chromosome> chromosomes) {
        for(Chromosome chromosome : chromosomes) {
    ...
    public static Chromosome getCmax(List<Chromosome> chromosomes) {
        ...
        //ChromosomeShaker.setCmaxes(chromosomes);
        for(Chromosome chromosome : chromosomes) {
